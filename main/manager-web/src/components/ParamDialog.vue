...
export default {
  ...
  methods: {
    submit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          // If it's an array type, validate the format and convert
          if (submitData.valueType === 'array' && submitData.paramValue) {
            ...
            for (let i = 0; i < lines.length - 1; i++) {
              if (!lines[i].trim().endsWith(';')) {
                this.$message.error('Invalid array format. Each line must end with a semicolon.');
                return;
              }
            }
          ...
          // If it's a json type, minify the JSON before submitting
    ...
    // Exposed for the parent component to reset the saving state
    resetSaving() {
      this.saving = false;
    }
  },
    ...
    visible(newVal) {
      if (newVal) {
        if (this.form.paramValue) {
          // If it's a json type, format it for display
          ...
          // If it's an array type, convert the semicolon-separated string to one item per line
        ...
        // Reset the saving state when the dialog is closed
        this.saving = false;
      }
    }
  }
