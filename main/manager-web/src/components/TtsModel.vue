...
export default {
  ...
  methods: {
    ...
    cancelEdit(row) {
      // Data created via Add must be removed from the array when editing is cancelled
      if (!row.id) {
    ...
    saveEdit(row) {
      ...
      try {
        ...
        // Also restore the original data on error
        if (row.originalData) {
          Object.assign(row, row.originalData);
          row.editing = false;
          delete row.originalData;
...
/* Special handling for the audio input field */
