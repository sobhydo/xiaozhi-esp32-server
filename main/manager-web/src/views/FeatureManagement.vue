...
export default {
  ...
  data() {
    return {
      ...
      isSaving: false // Add saving state lock
    }
  },
  computed: {
  ...
  methods: {
    ...
    async loadFeatures() {
      ...
      // Restore the user's selection state (if any)
      const allFeatures = [...this.featureManagementFeatures, ...this.voiceManagementFeatures]
    ...
    async toggleFeature(feature) {
      // If a save is in progress, block the operation
      if (this.isSaving) {
        return
      }
      ...
      // No longer update the config manager immediately; only update all at once on save
    },
    // Save config
    async handleSave() {
      ...
      // Set the saving state to lock the UI
      this.isSaving = true

      try {
        // Get the current state of all features and save them
        ...
        // Whether it succeeds or fails, release the saving state lock
        this.isSaving = false
      }
    },
    // Set up the config change listener
    ...
    // Remove the config change listener
