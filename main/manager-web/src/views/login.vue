...
export default {
  ...
  computed: {
    ...
    // Get the matching xiaozhi-ai icon based on the current language
    xiaozhiAiIcon() {
      const currentLang = this.currentLanguage;
      switch (currentLang) {
        case "zh_CN":
  ...
  mounted() {
    ...
    this.$store.dispatch("fetchPubConfig").then(() => {
      // Decide the default login method based on the config
  ...
  methods: {
    ...
    fetchCaptcha() {
      // Handle the case where manually clearing localStorage prevents fetching the captcha
    ...
    // Handle visibility changes of the language dropdown menu
    ...
    // Wrap the input validation logic
    ...
    getUserInfo() {
      Api.user.getUserInfo(({ data }) => {
        if (data.code === 0) {
          ...
          showDanger("Failed to retrieve user information");
        }
      });
    },
    ...
    async login() {
      if (this.isMobileLogin) {
        // Mobile phone login validation
        if (!validateMobile(this.form.mobile, this.form.areaCode)) {
          showDanger(this.$t('login.requiredMobile'));
          return;
        }
        // Concatenate the phone number as the username
      ...
      Api.user.login(
        ...
        (err) => {
          // Use the i18n message returned by the backend directly
