...
export default {
  ...
  methods: {
    ...
    handleSearch(keyword) {
      ...
      this.isLoading = true;
      // Detect MAC address format: contains colons
      ...
      Api.agent.searchAgent(keyword, searchType, ({ data }) => {
        ...
        console.error('Failed to search agent:', error);
        this.isLoading = false;
    ...
    handleSearchReset() {
      ...
      // Assign the original device list directly to the displayed list to avoid reloading data
      this.devices = [...this.originalDevices];
    },

    // Update the agent list from search
    ...
    fetchAgentList() {
      ...
      Api.agent.getAgentList(({ data }) => {
        if (data?.data) {
          ...
          // Dynamically set the skeleton count (optional)
...
/* In the styles of DeviceItem.vue */
.device-item {
  margin: 0 !important;
  /* Avoid conflicts */
