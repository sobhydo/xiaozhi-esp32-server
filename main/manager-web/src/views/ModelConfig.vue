...
export default {
  ...
  mounted() {
    // After the component is mounted, ensure the header translation text displays correctly
    setTimeout(() => {
  ...
  updated() {
    // After the component updates, reset the header translation text
    this.updateSelectionHeaderText();
  },
  ...
  methods: {
    // Update the selection column header translation text
    ...
    handleModelSave({ provideCode, formData, done }) {
      ...
      if (this.editModelData.duplicateMode) {
        ...
        Api.model.addModel({ modelType, provideCode, formData }, ({ data }) => {
          ...
          done && done(); // Call the done callback to clear the loading state
        });
      } else {
    ...
    // Fetch the model configuration list
    loadData() {
...
::v-deep .el-table .custom-selection-header .cell::before {
  ...
  text-align: center;
  line-height: 32px;
  /* Set an appropriate line height to ensure the text displays fully */
  color: black;
  margin-top: 0;
  /* Remove the top margin that could cause an offset */
  height: 32px;
  position: absolute;
  top: 50%;
  left: 50%;
...
/* Removed the empty pseudo-element that could affect text display */
