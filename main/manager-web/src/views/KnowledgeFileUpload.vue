...
export default {
  ...
  computed: {
    ...
    // Calculate slice pagination page count
    slicePageCount() {
      return Math.ceil(this.sliceTotal / this.slicePageSize);
    },
    // Calculate visible slice page numbers (show at most 3 page numbers)
    sliceVisiblePages() {
      const pages = [];
      const maxVisible = 3;
  ...
  methods: {
      ...
      KnowledgeBaseAPI.getDocumentList(this.datasetId, params,
        async ({ data }) => {
          ...
          if (data && data.code === 0) {
            ...
            // Fetch slice count for each document
            await this.fetchSliceCountsForDocuments();

            // Automatically start status polling for documents being processed
        ...
        (err) => {
          ...
          if (err && err.data) {
            console.log('Error message returned by backend:', err.data.msg || err.msg);
    ...
    startStatusPolling: function () {
      // Check whether polling is already running
      if (this.statusPollingTimer) {
        console.log('Status polling is already running');
        return;
      }

      // Check whether there are documents being processed
      const hasProcessingDocuments = this.fileList.some(document =>
        document.parseStatusCode === 1
      );

      if (!hasProcessingDocuments) {
        console.log('No documents being processed, status polling will not start');
        return;
      }
    ...
    pollDocumentStatus: async function () {
      // Check whether the maximum polling time has been exceeded
      ...
      try {
        ...
        if (response && response.code === 0) {
          ...
          // Check whether there are still documents being processed
    ...
    updateDocumentStatuses: function (updatedFileList) {
      ...
      updatedFileList.forEach(updatedDoc => {
        ...
        if (existingDoc && existingDoc.parseStatusCode !== updatedDoc.parseStatusCode) {
          // Status changed, update the document
          ...
          // If the status becomes completed, start slice count detection
    ...
    // Fetch slice count for each document in the file list
    ...
    // Fetch the slice count for a single document
    fetchSliceCountForSingleDocument: function (documentId) {
      ...
      const params = {
        page: 1,
        page_size: 1  // We only need the total count, so one record per page is enough
      };
      ...
      KnowledgeBaseAPI.listChunks(this.datasetId, documentId, params,
        ({ data }) => {
          if (data && data.code === 0) {
            ...
            // Update the document's slice count
            this.$set(document, 'sliceCount', sliceCount);
            // Force a view update
            this.$forceUpdate();
            console.log(`Slice count for document ${document.name} updated to:`, sliceCount);
          } else {
    ...
    // Smartly detect slice generation status and refresh automatically
    smartRefreshSliceCount: function (documentId) {
      ...
      // Wait 2 seconds before fetching the slice count to give the backend more processing time
      setTimeout(() => {
    ...
    showUploadDialog: function () {
      ...
      this.selectedFilesList = []; // Clear the selected files list
    ...
    beforeUpload: function (file) {
      ...
      // Save the file to uploadForm
      this.uploadForm.file = file;
      return false; // Prevent automatic upload and use custom upload logic
    },
    ...
    handleBatchUploadSubmit: function () {
      ...
      // Create the array of upload tasks
      const uploadPromises = this.selectedFilesList.map(file => {
        return new Promise((resolve, reject) => {
          ...
          KnowledgeBaseAPI.uploadDocument(this.datasetId, formData,
            ...
            (err) => {
              // Error callback handles the error message returned by the backend
              if (err && err.data) {
      ...
      Promise.all(uploadPromises.map(p => p.catch(e => e)))
        .then(results => {
          ...
          if (successCount > 0) {
            this.$message.success(`Successfully uploaded ${successCount} file(s)`);
          }

          if (failedCount > 0) {
            const failedFiles = results.filter(r => !r.success).map(r => r.fileName);
            this.$message.error(`Failed to upload ${failedCount} file(s): ${failedFiles.join(', ')}`);
          }
    ...
    // Single file upload (legacy feature retained)
    ...
    handleParse: function (row) {
      this.$confirm(this.$t('knowledgeFileUpload.confirmParse'), this.$t('warning'), {
        ...
        KnowledgeBaseAPI.parseDocument(this.datasetId, row.id,
          ({ data }) => {
            if (data && data.code === 0) {
              this.$message.success('Request submitted, parsing in progress');

              // Immediately update the document status to processing
              ...
              // Use smart detection to refresh the slice count automatically
    ...
    // Jump to the first page of slice management
    ...
    // Previous page of slice management
    ...
    // Jump to a specific page of slice management
    ...
    // Retrieval test methods
    showRetrievalTestDialog: function () {
      // Initialize the retrieval test form
...
.main-wrapper {
  // Top 63px, bottom 35px, search bar 58px
...
/* Fixed container size for the slice management dialog */
.slice-dialog {
  ::v-deep .el-dialog__wrapper {
    display: block !important;
  }
  ...
  .slice-card {
    ...
    .slice-card-content {
      ...
      .content-text {
        ...
        /* Ensure text displays correctly, including spaces and line breaks */
        white-space: pre-wrap;
